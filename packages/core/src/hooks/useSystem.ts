...
function useFixedTimestep(updateCallback: UpdateCallback) {
  ...
  const update = useCallback(
    function useFixedTimestepUpdate(t: Ticker) {
      ...
      world.set(WorldTraits.Delta, {
        deltaTime: totalSimulationTime,
        fps: 1 / totalSimulationTime,
        dilation: Math.min(TARGET_FPS, t.FPS) / TARGET_FPS,
      });
    },
...
export function useSystem(system: System) {
  ...
  useTick(useFixedTimestep(updateSystem));
}
...
export function useSystems(systems: System[]) {
  ...
  useTick(useFixedTimestep(updateSystems));
}
